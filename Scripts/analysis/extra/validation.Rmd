...
#seurat <- readRDS(file = "/u/project/xyang123/ligaoyan/HypothalamusSexDiff/Data/Datawithmetainfo/Allcelltypes.rds")
...
#                                                id = DataFrame(label = sce$celltype, 
#                                                               sample = sce$Sample2))
```
...
pdf("Plots/Final_Figures/Validation/Xist_Uty_barplot_Sample2_legend.pdf", width = 16, height = 12)
...
sexing_df %>%
  ggplot(aes(x = UtyCounts, y = Sample, fill = Genotype)) + geom_bar(position = "dodge", stat = "identity") +
    ...
    ylab("Sample") +
  ggtitle("Fraction Uty Count by Sample")
ggsave(filename = "Plots/Validation/Uty_barplot.pdf", plot = last_plot(), width = 7, height = 5)
...
escape_genes <- gene_annotation %>% dplyr::filter(Escapee == "Escapee")


seurat$Genotype <- factor(seurat$Genotype, levels = c("XYM", "XYF", "XXM", "XXF", "XYYM", "XYYF", "XXYM", "XXYF"))
seurat_gt_list <- SplitObject(seurat, split.by = "Genotype")
plotlist <- lapply(seurat_gt_list, function(x, n) {
  plot <- VlnPlot(x, features = c("Xist", "Ddx3x"), pt.size = 0, group.by = "sample2", ncol = 2)
  return(plot)
})
png("Plots/Validation/MS_escape_vlnplot.png", height = 50, width = 10, units = "in", res = 600)
ggarrange(plotlist = plotlist, ncol = 1, nrow = 8)
dev.off()
...
VlnPlot(seurat, features = escape_genes$mgi_symbol, pt.size = 0, group.by = "sample2", ncol = 5)
ggsave(filename = "Plots/Validation/MS_escape_vlnplot.png", plot = last_plot(), height = 15, width = 28)

VlnPlot(seurat, features = c("Xist", "Uty"), ncol = 2, pt.size = 0, group.by = "sample2")
ggsave(filename = "Plots/Validation/MS_xist_uty_vlnplot.png", plot = last_plot(), height = 5, width = 15)
...
pdf("Plots/Validation/hypothalamus_Escape_barplots.pdf", width = 30, height = 15)
