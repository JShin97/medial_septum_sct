...
seurat$celltype <- gsub("_", "-", seurat$celltype)
...
metadata <- seurat@meta.data %>% dplyr::select(SampleID, Genotype, Gonad, SexChrom, Trisomy, addX, GenotypeaddX, addY1, addY2, GenotypeaddY, Sequencing_Date)
rownames(metadata) <- NULL
metadata <- unique(metadata)
summed_seurat@meta.data <- summed_seurat@meta.data %>% 
  rownames_to_column(var = "ID") %>% 
  left_join(metadata, by = c("orig.ident" = "SampleID")) %>% 
  dplyr::mutate(celltype = map_chr(str_split(ID, "_"), 2)) %>% 
...
summed <- aggregateAcrossCells(sce, 
                               id = colData(sce)[, c("celltype", "SampleID")])
summed$id <- paste(summed$celltype, summed$orig.ident, sep = "_")

cols_to_keep <- c("id", "SampleID", "Genotype", "celltype", "Gonad", "SexChrom", "Trisomy", "ncells")
...
for (i in 1:length(allcells)) {
  ...
  cell_metadata <- metadata[metadata$SampleID %in% rownames(dge0$samples), ]
...
allcells <- unique(summed_seurat$celltype)
allcells <- allcells[!allcells %in% "Misc-NT"]
for (i in 1:length(allcells)) {
  ...
  design <- model.matrix(~ Genotype + Sequencing_Date, data=dge$samples)
  y <- new("EList")
  ...
  save(resultlist_normative, file = paste0("Results/complete_analysis/DEG_SD_PB/normative/", currentcell,".rda"))
 
  # resultlist_normative_block <- list()
  # design <- model.matrix(~ Genotype + Sequencing_Date, data=dge$samples)
  # y <- new("EList")
  # v <- voom(dge, design)
  ...
  # v <- voom(dge, design, block = dge$samples$SampleID, correlation = dupcor$consensus.correlation)
  # dupcor <- duplicateCorrelation(v, design, block = dge$samples$SampleID)
  # vfit <- lmFit(v, design = design, block = dge$samples$SampleID, correlation = dupcor$consensus.correlation)
  # efit <- eBayes(vfit)
  # 
  # deg_results <- topTable(efit, n = Inf, adjust.method = "BH", coef = "GenotypeXXF", confint = TRUE) %>% 
  #   dplyr::mutate(SE = (CI.R - CI.L)/3.92)
  # 
  # resultlist_normative_block[["XXF_vs_XYM"]] <- deg_results
  ...
  resultlist_fcg <- list()
  design <- model.matrix(~ Gonad + SexChrom + Gonad:SexChrom + Sequencing_Date, data=dge_samples)
  y <- new("EList")
  ...
  save(resultlist_fcg, file = paste0("Results/complete_analysis/DEG_SD_PB/fcg/", currentcell,".rda"))
  
  #addX 
  dge_samples <- dge$samples[dge$samples$SexChrom %in% c("XY", "XXY"), ]
  dge_samples$GenotypeaddX <- factor(dge_samples$GenotypeaddX, levels = c("XYM", "XYF", "XXYM", "XXYF"))
  dge_samples$addX <- as.numeric(dge_samples$addX)
  dge_counts <- dge$counts[, colnames(dge$counts) %in% rownames(dge_samples)]
  
  resultlist_addx <- list()
  design <- model.matrix(~ GenotypeaddX + addX + Sequencing_Date, data=dge_samples)
  y <- new("EList")
  y$E <- edgeR::cpm(dge_counts, log = TRUE, prior.count = 3)
  fit <- lmFit(y, design = design)
  fit <- eBayes(fit, trend = TRUE, robust = TRUE)
  ...
  save(resultlist_addx, file = paste0("Results/complete_analysis/DEG_SD_PB/addx/", currentcell,".rda"))
  
  #addY1
  dge_samples <- dge$samples[dge$samples$Genotype %in% c("XXM", "XXYM", "XXF", "XXYF"), ]
  dge_samples$GenotypeaddY <- factor(dge_samples$GenotypeaddY, levels = c("XXM", "XXF", "XXYM", "XXYF"))
  dge_samples$addY1 <- as.numeric(dge_samples$addY1)
  dge_counts <- dge$counts[, colnames(dge$counts) %in% rownames(dge_samples)]
  
  resultlist_addy1 <- list()
  design <- model.matrix(~ GenotypeaddY + addY1 + Sequencing_Date, data=dge_samples)
  y <- new("EList")
  y$E <- edgeR::cpm(dge_counts, log = TRUE, prior.count = 3)
  fit <- lmFit(y, design = design)
  fit <- eBayes(fit, trend = TRUE, robust = TRUE)
  ...
  save(resultlist_addy1, file = paste0("Results/complete_analysis/DEG_SD_PB/addy1/", currentcell,".rda"))
  
  #addY2
  dge_samples <- dge$samples[dge$samples$Genotype %in% c("XYM", "XYYM", "XYF", "XYYF"), ]
  dge_samples$GenotypeaddY <- factor(dge_samples$GenotypeaddY, levels = c("XYM", "XYF", "XYYM", "XYYF"))
  dge_samples$addY2 <- as.numeric(dge_samples$addY2)
  dge_counts <- dge$counts[, colnames(dge$counts) %in% rownames(dge_samples)]
  
  resultlist_addy2 <- list()
  design <- model.matrix(~ GenotypeaddY + addY2 + Sequencing_Date, data=dge_samples)
  y <- new("EList")
  y$E <- edgeR::cpm(dge_counts, log = TRUE, prior.count = 3)
  fit <- lmFit(y, design = design)
  fit <- eBayes(fit, trend = TRUE, robust = TRUE)
  ...
  save(resultlist_addy2, file = paste0("Results/complete_analysis/DEG_SD_PB/addy2/", currentcell,".rda"))
