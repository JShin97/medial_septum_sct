...
#
# Written by Zeyneb Kurt 2015, updated by Yanning Zuo 2020
#
kda2cytoscape <- function(job, node.list=NULL, modules=NULL, ndrivers, 
depth=1) {
    # Select top scoring modules.
    ...
    palette <- kda2cytoscape.colormaps(length(mods))
    ...
    #Added in by Zara Saleem 
    #####################################################################################
    ### updated on May16th, 2018 for keeping the original weights from the given network:
    ...
    if(direction > 0)
    	edge.weights.to.be.found=merge(edge.weights.to.be.found, whole.network, by.x=c(1,2), by.y=c(1,2))
    if(direction < 0)
    	edge.weights.to.be.found=merge(edge.weights.to.be.found, whole.network, by.x=c(1,2), by.y=c(2,1))
    if(direction == 0)
    	edge.weights.to.be.found=unique(rbind(merge(edge.weights.to.be.found, whole.network, by.x=c(1,2), by.y=c(1,2)), 
    							merge(edge.weights.to.be.found, whole.network, by.x=c(1,2), by.y=c(2,1))))
    ...
    edge.weights.to.be.found=merge(edgdata.wout.wgts, edge.weights.to.be.found, by.x=c(1,2), by.y=c(1,2))
    edge.weights.to.be.found= edge.weights.to.be.found[,c(1,2,5,3,4)]
    
    edgfile2 <- file.path(dpath, "kda2cytoscape.edges.with.weights.txt")
    if (!is.null(edge.weights.to.be.found)) tool.save(frame= edge.weights.to.be.found, file=edgfile2)

    #####################################################################################
    

    if (!is.null(noddata)) 
        tool.save(frame=noddata[, c("NODE","MEMBERSHIP","URL", "LABEL", "SIZE","COLOR", "SHAPE", "FONT",
         "MODULE")], file=nodfile)
...
kda2cytoscape.exec <- function(i,job, drivers, modpool, graph.depth=1,palette){
    ...
    # Select affected nodes.
    tmp <- data.frame(unique(c(edgdata$TAIL, edgdata$HEAD)),stringsAsFactors=FALSE)
    names(tmp) <- "NODE"
    # Assign node shapes.
...
kda2cytoscape.membership <- function(i,noddata, moddata, modpool, palette) {
    ...
    if(length(noddata$NODE) > 0){
        ...
        res <- data.frame(NODE=noddata$NODE, COLOR=colors, SECTOR=sectors, URL=urls, LABELSIZE=label.size, MEMBERSHIP = mem, stringsAsFactors=FALSE)
        res <- merge(noddata, res, all.x=TRUE)
...
kda2cytoscape.colormaps <- function(ncolors) {
    library(ggtern)
    palette <- rainbow(n=(ncolors + 2))
    palette <- col2rgb(palette)
    
    # Lighten raw colors YZ 01/2020
    factor = 0.7
    palette <- palette + (255 - palette)*factor
    
    #YZ: randomize colors
    palette <- round(palette)
    palette <- rgb2hex(palette[1,],palette[2,],palette[3,])
