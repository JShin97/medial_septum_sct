...
cellTypeCount <- function(seuratObject, meta_data_1 = "Cell_type", meta_data_2 = "orig.ident", includePerc = TRUE){
  ...
  if(length(unique(seuratObject@meta.data[,meta_data_2]))!=length(levels(seuratObject@meta.data[,meta_data_2]))){
    idents_table <- idents_table[idents_table$Var2 %in% unique(seuratObject@meta.data[,meta_data_2]),]
  }
...
convertMouseGeneList <- function(x){
  ...
  #TODO: implement this version of the useMart command to specify where data is retrieved from
  ...
  human = useMart(biomart="ensembl",
                  dataset="hsapiens_gene_ensembl", host = "https://dec2021.archive.ensembl.org/")
...
volcano_plot <- function(DEG_df, 
                         cell_type=NULL, 
                         color_by=NULL, # color_by is column that contains meta data you want to color by
                                        #if no color_by set, upregulated genes are red and downregulated genes are blue
                         colors = NULL, # character vector of colors, with each corresponding to a category of the meta var used for color by
                                        # cannot be more than 8 levels. You shouldn't visualize more than 8 anyway!
                         lbl_size = 3,
                         lfc = .25, #log Fold change cutoff for determining which genes are labelled
                         padjusted = 0.001, #p_adjusted value cutoff for determining which genes are labelled
                         indicate_stringency = FALSE, # requires having run samplewise DEGs
...
summarizeTopDegs <- function(DEG_df, 
  ...
  for(comp in names(comparisons)){
      ...
      if(sum(subset$GENE==DEG)>0){
        ...
        celltype_group = append(celltype_group,
                                concatenate(gsub(paste0(comp,"_"),"",subset[,MODULE][subset$GENE==DEG]),
                                            mysep = ", "))
        #print("haha2")
        if("avg_logFC" %in% colnames(subset)){
    ...
    top_DEGs = data.frame("GENE" = genes,
...
#David S Edit: remove this comment later
#   Allow toggle for convertGeneHuman t
#   added new column, alignment_score, which is the relative overlap between module and pathway ()
markerGeneOverlap <- function(query_markers, 
  ...
  overlap <- makePathwayEnrichmentDf(DEG_df=query_markers, 
                                     ...
                                     convertToHuman=convertToHuman,
                                     plot_wid = overlap_plot_wid, 
                                     plot_hgt = overlap_plot_hgt)
  #WIP new metric, alignment score
  overlap$alignment_score =  
    (overlap$nOverlap/overlap$PathwayGeneCount + overlap$nOverlap/overlap$ModuleGeneCount)*50
  ...
  for(mod in unique(overlap$Module)[order(unique(overlap$Module))]){
    ...
    #WIP new metric
    byAlignment <- subset$Pathway[subset$alignment_score==max(subset$alignment_score)]
    byAlignment <- paste(byAlignment, collapse = ", ")
    temp <- data.frame("Cluster"=mod, 
                       "ByOverlapPrediction"=byOverlap, 
                       "ByEnrichmentPrediction"=byEnrichment, 
                       "ByFDRPrediction"=byFDR,
                       "ByAlignmentScore"=byAlignment)
