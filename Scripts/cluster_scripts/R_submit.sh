...
echo "/usr/bin/time -v apptainer exec $H2_CONTAINER_LOC/h2-rstudio_4.3.0.sif R CMD BATCH --no-save --no-restore MSEA_correctmapping.R logs/output.$JOB_ID"
/usr/bin/time -v apptainer  exec $H2_CONTAINER_LOC/h2-rstudio_4.3.0.sif R CMD BATCH --no-save --no-restore MSEA_correctmapping.R logs/output.$JOB_ID
